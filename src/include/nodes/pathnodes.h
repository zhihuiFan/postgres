...
/*
 * UniqueKey in RelOptInfo represents a set of Exprs are Unique by
 * nature. The "unique" here is same as "unique" in "unique index",
 * it allows multiple null values exists. If we want to check if
 * the exprs may be NULL, RelOptInfo.notnull_attrs can be consulted.
 *
 * Each expr in UnqiueKey is an EquivalenceClass, to save storage,
 * Bitmapset * is used. Each bit presents the position of the EC
 * in root->eq_classes. SingleRow is a special case of UniqueKey,
 * every expr in this RelOptInfo is unique since only 1 row exists.
 * SingleRow can be generated by a case like WHERE pk = 1; To present
 * more effectively, we use `int relid` in UniqueKey to present every
 * expression for RelOptInfo(relid) is unique in current RelOptInfo.
 *
 * Some step by step observations:
 * 0). If a is uniquekey, (a, any_column) is unique.
 * 1). For a BaseRel, it can be a singlerow, in this case eclass_indexes
 * equals NULL. OR singelrow equals 0, and eclass_indexes should
 * be set.
 * 2). During join two relations
 * 2.1). If one rel is singlerow, then the other rel is impossible get
 * duplicated because of "any kind of join", so the UniqueKey in the other
 * rel is still valid.
 * 2.2). if the_other_rel.any_column = rel.unique_expr, the_other_rel's
 * UniqueKey still valid.
 * 2.3). The composite UniqueKey from both sides is still a valid UniqueKey
 * for UniqueKey for "any kind of join".
 * 2.4). Because of 2.1 & 0, any sides of composited UniqueKey are impossible
 * to SingleRow.
 *
 * Similar with PathKey, only interesting UniqueKey is maintained. Luckily
 * all the interesting case, there is a EC created already for UniqueKey.
 */

typedef struct UnqiueKey
{
	NodeTag		type;
	Bitmapset	*eclass_indexes;	/* Indexes in PlannerInfo's eq_class list of ECs that
									 * is unique for a RelOptInfo.
									 */
	int			relid;
