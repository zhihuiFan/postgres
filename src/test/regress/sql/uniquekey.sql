...
SELECT DISTINCT t1.c, t1.d, t2.pk FROM uqk1 t1 cross join uqk2 t2 wheret t1.c is not null order by 1, 2, 3 ;
SELECT t1.c, t1.d, t2.pk FROM uqk1 t1 cross join uqk2 t2 wheret t1.c is not null order by 1, 2, 3;
...
EXPLAIN SELECT DISTINCT v.* FROM
(SELECT t1.c, t1.d, t2.pk FROM uqk1 t1 cross join uqk2 t2 where t1.c is not null)
v;

-- TEST EC
EXPLAIN SELECT DISTINCT v.t2c,  FROM
(SELECT DISTINCT uqk1.c, uqk1.d FROM uqk1, uqk2
WHERE uqk1.a = uqk2.pk offset 0) v;

-- test a SingleRow case.
EXPLAIN SELECT DISTINCT v.t2c FROM
(SELECT t1.c, t1.d, t2.c as t2c
 FROM uqk1 t1 cross join uqk2 t2
 WHERE t1.c is not null AND t2.pk = 1
 OFFSET 0) v;
