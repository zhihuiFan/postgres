...
AppendSeconds(char *cp, int sec, fsec_t fsec, int precision, bool fillzeros)
{
	...
	if (fillzeros)
		cp = pg_int32tostr_nn_zeropad(cp, Abs(sec), 2);
	else
		cp = pg_int32tostr_nn(cp, Abs(sec));
	...
	if (fsec != 0)
	{
		...
		*cp++ = '.';
		 ...
		 * enough to print the number.  We punt the problem to pg_int32tostr_nn(),
		 * which will generate a correct answer in the minimum valid width.
		 */
		if (value)
			return pg_int32tostr_nn(cp, Abs(fsec));
...
EncodeTimezone(char *str, int tz, int style)
{
	...
	*str++ = (tz <= 0 ? '+' : '-');
	...
	if (sec != 0)
	{
		str = pg_int32tostr_nn_zeropad(str, hour, 2);
		*str++ = ':';
		str = pg_int32tostr_nn_zeropad(str, min, 2);
		*str++ = ':';
		str = pg_int32tostr_nn_zeropad(str, sec, 2);
	}
...
EncodeDateOnly(struct pg_tm *tm, int style, char *str)
{
	...
	switch (style)
	{
		...
		case USE_XSD_DATES:
			...
			str = pg_int32tostr_nn_zeropad(str,
									(tm->tm_year > 0) ? tm->tm_year : -(tm->tm_year - 1), 4);
			*str++ = '-';
			str = pg_int32tostr_nn_zeropad(str, tm->tm_mon, 2);
			*str++ = '-';
			str = pg_int32tostr_nn_zeropad(str, tm->tm_mday, 2);
			break;
...
{
	str = pg_int32tostr_nn_zeropad(str, tm->tm_hour, 2);
	*str++ = ':';
	str = pg_int32tostr_nn_zeropad(str, tm->tm_min, 2);
	*str++ = ':';
